    ...
    fn parse_line(line: &str) -> Result<Self> {
        ...
        for i in 0..parts.len() {
            match parts[i] {
                ...
                "calories" => calories = parts[i + 1].parse()?,
                _ => continue,
            }
        }
...
fn calculate_score(ingredients: &[Ingredient], amounts: &[i32; 4]) -> (i32, i32){
    let mut capacity = 0;
    let mut durability = 0;
    let mut flavor = 0;
    let mut texture = 0;
    ...
    for (ingredient, &amount) in ingredients.iter().zip(amounts.iter()) {
        ...
        calorie += ingredient.calories * amount;
    }
...
pub fn solve() -> Result<i32> {
    ...
    // a + b + c + d == 100 AND total calories == 500
    for a in 0..=100 {
        for b in 0..=(100 - a) {
            for c in 0..=(100 - a - b) {
                let d = 100 -a -b -c;
