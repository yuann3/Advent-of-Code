...
fn parse_coordinate(string: &str) -> Result<(usize, usize)> {
...
fn parse_instructions(line: &str) -> Result<Command> {
    ...
    let end = parse_coordinate(words.last().ok_or_else(|| anyhow!("Missing coordinates"))?)?;
